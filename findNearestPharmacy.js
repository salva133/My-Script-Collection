...
async function loadData() {
  const responseJson = await fetch('/dam/jcr:f4fcfc72-3f6b-49f6-84a9-258af95424d5/pharmacies.json');
  const dataJson = await responseJson.json();
  arrJson = dataJson;

  const responsePlz = await fetch('/dam/jcr:941d9e94-7907-4981-b600-20e2252e6f8a/plz_germany.json');
...
function debugVariableObj(variableObj) { // Usage: debugVariableObj({ <variablename> })
    if (DEBUG_MODE) {
        if (!variableObj || Object.keys(variableObj).length === 0) {
            console.log("[DEBUG] No object provided. Please check your input. Correct usage: debugVariableObj({ variableName });");
            return;
        }
...
function addPlz() {
  ...
  if (locations.length > 0) {
    let apotheke = locations.map(location => 
      encodeURIComponent(removeSpecialChars(decodeHtml(location.Apothekenname)))).join('|');
    ...
    let location = arrJson.reduce((prev, curr) => {
        const prevDistance = getDistanceFromLatLonInKm(initialLat, initialLng, prev.lat, prev.lng);
        const currDistance = getDistanceFromLatLonInKm(initialLat, initialLng, curr.lat, curr.lng);

        return currDistance < prevDistance ? curr : prev;
    });
    if (location) {
      let apotheke = encodeURIComponent(removeSpecialChars(decodeHtml(location.Apothekenname)));
