...
def check_video_files():
    video_files = [f for f in os.listdir() if f.endswith((".mp4", ".mov", ".mkv", ".webm", ".avi", ".flv", ".wmv", ".mpeg", ".mpg", ".m4v", ".3gp", ".3g2", ".ogv", ".vob")) and not f.startswith("data_")]
    if not video_files:
...
def check_extracted_folder():
    if os.path.isdir("extracted"):
        extracted_files = [f for f in os.listdir("extracted") if f.endswith('.jpg') or f.endswith('.png')]
        if extracted_files:
...
def main():
    video_files = check_video_files()                       # Check for video files in the current working directory
    check_extracted_folder()                                # Check if 'extracted' folder exists and if it contains image files
    total_seconds = calculate_total_seconds(video_files)    # Calculate the total seconds of the videos
    print_total_seconds(total_seconds)                      # Print the total number of seconds of the videos
    print_multiple_seconds(total_seconds)                   # Print the total seconds of the videos multiplied by 2-15
