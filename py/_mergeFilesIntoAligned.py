...
def move_files_from_subdirectories(cwd, aligned_dir):
    ...
    for subdir in subdirs:
        subdir_path = os.path.join(cwd, subdir)
        total_files += len([f for f in os.listdir(subdir_path) if os.path.isfile(os.path.join(subdir_path, f))])
...
def main():
    ...
    aligned_dir = create_directory(os.path.join(cwd, 'aligned'))        # Create the 'aligned' directory
    backup_files(cwd)                                                   # Backup files in the current working directory
    duplicate_files = move_files_from_subdirectories(cwd, aligned_dir)  # Move files from subdirectories to the 'aligned' directory
    print_duplicate_files(duplicate_files)                              # Print the duplicate files that were skipped due to name conflicts
    remove_empty_directories(cwd)                                       # Remove empty directories in the current working directory
    print("Files have been successfully moved to the 'aligned' folder. Empty folders have been deleted.")
